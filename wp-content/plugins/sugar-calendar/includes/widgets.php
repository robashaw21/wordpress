...
class sc_events_widget extends WP_Widget {
    ...
    {
        parent::WP_Widget(false, __('Event Calendar', 'pippin_sc'), array('description' => __('Displays an events widget', 'pippin_sc')));
    }
...
class sc_events_list_widget extends WP_Widget {
    ...
    /** constructor */
    function sc_events_list_widget()
    {
        parent::WP_Widget(false, __('Events List', 'pippin_sc'), array('description' => __('Displays upcoming/past events in a list view', 'pippin_sc')));
    }
    ...
    function widget($args, $instance)
    {
        ...
        echo '<div id="sc_list_wrap">';
			echo sc_get_events_list( $display, $category, $number );
        echo '</div>';
    ...
    function form($instance)
    {
    	...
    	<p>
        	<select name="<?php echo $this->get_field_name('display'); ?>" id="<?php echo $this->get_field_id('display'); ?>">
				...
				<option value="upcoming" <?php selected('upcoming', $display); ?>><?php _e('Upcoming', 'pippin_sc'); ?></option>
				<option value="past" <?php selected('past', $display); ?>><?php _e('Past', 'pippin_sc'); ?></option>
			</select>
        	<label for="<?php echo $this->get_field_id('display'); ?>"><?php _e('Past or Upcoming', 'pippin_sc'); ?></label>
    	</p>
		<p>
			<select name="<?php echo $this->get_field_name('category'); ?>" id="<?php echo $this->get_field_id('category'); ?>">
				<option value="0" <?php selected(0, $category); ?>><?php _e('All', 'pippin_sc'); ?></option>
		...
		<p>
        	<input class="widefat" id="<?php echo $this->get_field_id('number'); ?>" style="width: 40px;"
               name="<?php echo $this->get_field_name('number'); ?>" type="text" value="<?php echo $number; ?>"/>
        	<label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('Number to show', 'pippin_sc'); ?></label>
    	</p>
    <?php
    }
}
...
class sc_event_categories_widget extends WP_Widget {
    ...
    /** constructor */
    function sc_event_categories_widget()
    {
        parent::WP_Widget(false, __('Event Categories', 'pippin_sc'), array('description' => __('Display the event categories', 'pippin_sc')));
    }
...
class sc_event_filter_widget extends WP_Widget {
    ...
    /** constructor */
    function sc_event_filter_widget()
    {
        parent::WP_Widget(false, __('Event Filter', 'pippin_sc'), array('description' => __('Filtering controls for upcoming and past events display. Use only on /events archive.', 'pippin_sc')));
    }
