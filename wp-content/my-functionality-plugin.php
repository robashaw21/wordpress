	...
	ob_start();
		...
		if ( $events ) {
			...
			foreach( $events as $event ) {
				...
				echo"		<input name='input_" . $fieldID . "' type='radio' value='".date_i18n( "Y.m.d", sc_get_event_date($event->ID, false))."' id='choice_" . $fieldID . "_".$optionNum."'".GFCommon::get_tabindex()." disabled='disabled' ";
				if(date_i18n( "Y.m.d", sc_get_event_date($event->ID, false)) == apply_filters('gform_field_value_scEventDate', '')) { 
					echo "checked='checked'>"; 
				} else { 
					echo ">";
				}
				echo date_i18n( "Y.m.d", sc_get_event_date($post->ID, false));
